...
def load_config():
    with open("../../vvmsd_config.yaml") as file:
...
@app.post("/prepare_txt2img")
async def f_pre_txt2img(req: str):
    
    opt = load_config()['txt2img']

    sampler_index = get_sampler_index(opt['sampler_name'])

    seed = opt['seed']
    #if req.seed is not None and not req.seed == '':
    #    seed = int(req.seed)

    width, height = fix_aspect_ratio(opt['base_size'], opt['max_size'],
                                     opt['base_size'], opt['base_size'])
    ...
    sample_path = opt['sample_path']
    os.makedirs(sample_path, exist_ok=True)
    resized_images = [images.resize_image(0, image, opt['base_size'], opt['base_size']) for image in output_images]
    outputs = [save_img(image, sample_path, filename=f"{int(time.time())}_{i}.png")
               for i, image in enumerate(resized_images)]
    print(f"finished: {outputs}\n{info}")
    return FileResponse(path=outputs[0], media_type="image/png")
...
async def f_img2img(req: Img2ImgRequest):
    ...
    imgbytes = base64.b64decode(req.imgdata)
    #print(imgbytes)
...
def main():
    uvicorn.run("vvmsd_server:app", host="127.0.0.1", port=USER_PORT, log_level="info")
